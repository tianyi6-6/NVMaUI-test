...
class DataModel(QObject):
    data_updated = Signal(list)  # 每次采集数据后发出信号

    def __init__(self, LIA_mini_API):
        super().__init__()
        self.LIA_mini_API = LIA_mini_API
        self._running = False

    def start(self):
        self._running = True
        self.LIA_mini_API.USB_START()
        self.LIA_mini_API.all_start()

        while self._running:
            fluo_data, laser_data = self.LIA_mini_API.auxDAQ_play(125)
            self.data_updated.emit(fluo_data)
            time.sleep(0.01)  # 采样间隔

    def stop(self):
        self._running = False
        self.LIA_mini_API.USB_END()
...
class ViewModel(QObject):
    ...
    def __init__(self):
        ...
        self.LIA_mini_API = API('COM1')


        self.model = DataModel(self.LIA_mini_API)
        self.thread = QThread()
...
class MainWindow(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("实时绘图界面 - MVVM 架构 + 状态管理")
        self.resize(800, 500)

        # ViewModel 实例化
        self.vm = ViewModel()
        self.vm.data_ready.connect(self.update_plot)
        self.vm.state_changed.connect(self.on_state_changed)

        # 图表区域
        self.plot_widget = pg.PlotWidget(title="实时数据")
        self.plot_curve = self.plot_widget.plot(pen='g')
        self.data_buffer = []

        # 状态与值标签
        self.label = QLabel("最新数据: --")
        ...
        self.start_btn.clicked.connect(self.vm.start)
    ...
    def update_plot(self, value):
        """接收数据更新信号，刷新图表和标签"""
        self.label.setText(f"最新数据均值: {np.mean(value) * 1000:.2f} mV")
        # self.label.setText(f"最新数据点数: {len(value)}")
        # self.data_buffer.append(value)
        self.data_buffer = self.data_buffer + value
    ...
    def on_state_changed(self, state):
        ...
        text_map = {
            SystemState.IDLE: "空闲",
            SystemState.RUNNING: "采集中",
            SystemState.STOPPED: "已停止"
        }
        self.state_label.setText(f"当前状态: {text_map[state]}")
