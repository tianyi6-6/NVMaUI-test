...
class LIA_API(SerialController):
    def __init__(self, port='COM11', *args, **kwargs):
        SerialController.__init__(self, port, *args, **kwargs)
        # todo: 注册新设备类型
        self.dev_ctrl = {
            'LIA': self.lia_play,
            'MW': self.mw_play,
    ...
    def thread_IIR_play(self, background_flag=False, socket_upload_flag=False, address=None, port=None, show_flag=True):
        ...
        if (not background_flag):
            while self.IIR_run_flag:
                print("Qsize & Count", self.show_data_queue.qsize(), self.raw_data_queue.qsize(), self.count,
                      time.time())
                time.sleep(1)
