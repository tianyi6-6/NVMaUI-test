...
def readable_bytes(data: bytes, encoding='utf-8', errors='replace') -> str:
    """
    将bytes数据转换为可读字符串。

    - 可打印字符使用原始字符
    - 不可打印字符替换格式
    - 默认使用 UTF-8 解码，无法解码的字节以十六进制表示

    :param data: 要转换的bytes对象
    :param encoding: 解码编码（默认utf-8）
    :param errors: 解码错误处理方式（默认'replace'）
    :return: 可读字符串
    """
    result = []
    for b in data:
        try:
            char = bytes([b]).decode(encoding, errors=errors)
            if char.isprintable():
                result.append(char)
            else:
...
class SerialController:
    def __init__(self, port, baudrate=500000, bytesize=8, parity='N', stopbits=1, timeout=1.0):
        """
        初始化串口控制器。

        :param port: 串口号，如 'COM3' 或 '/dev/ttyUSB0'
        :param baudrate: 波特率，默认 9600
        :param bytesize: 数据位，通常为 5、6、7 或 8
        :param parity: 奇偶校验，'N'（无）、'E'（偶）、'O'（奇）
        :param stopbits: 停止位，1 或 2
        :param timeout: 读操作超时时间（秒）
        """
        self.port = port
        self.serial = None
        self.serial_kwargs = {
        ...
        self.bulk_buffer_packet_size = 512
        self.infinite_iir_buffer = b''
        self.infinite_pid_buffer = b''
        self.open_device()
    ...
    def open_device(self, *args, **kwargs):
        ...
        if self.serial and self.serial.is_open:
            return
        self.serial = serial.Serial(port=self.port, **self.serial_kwargs)
    ...
    def flush_input_buffer(self, max_empty_count=5, timeout_ms=100):
        ...
        # 读取所有当前缓冲区中的数据
        cached_data = self.serial.read_all()

        logging.info(f"清空缓存，读取到 {len(cached_data)} 字节")

    def flush_input_output(self):
        self.flush_input_buffer()

    def DWritePort(self, data: bytes, timeout_sec: float = 1.0):
        if self.serial and self.serial.is_open:
            # self.serial.timeout = timeout_sec
            # 调试用指令
            # print(f'>>下发指令 {str_to_hexstr(data)}')
            self.serial.write(data)
            # input(f'>>下发指令完成，等待按键继续：')


            # except:
            #     logging.error('写入数据失败。')

    def DReadPort(self, length: int, timeout_sec: float = 0.5) -> bytes:
        """读取指定长度数据"""
        buffer = bytearray()
        if not (self.serial and self.serial.is_open):
            return b'\x00' * length

        self.serial.timeout = timeout_sec # 每次 read 最多等 50ms，防止长期阻塞

        start_time = time.time()
        while len(buffer) < length:
            # 超时判断
            if (time.time() - start_time) > timeout_sec:
                logging.warning(f"串口读取超时，仅收到 {len(buffer)} / {length} 字节")
                break

            try:
                chunk = self.serial.read(min(length - len(buffer), self.bulk_buffer_packet_size))
                if chunk:
                    buffer.extend(chunk)
                    start_time = time.time()
                else:
    ...
    def DReadPortIIR(self, data_num: int, timeout_sec: float = 1.5):
        ...
        # IIR数据包格式：单帧84字节，帧头\xfe\xdc，12组6字节交流分量，2组4字节直流分量，1字节计数器，帧尾\xff
        FRAME_SIZE = 84
        FRAME_HEAD = b'\xfe\xdc'
        FRAME_TAIL = b'\xff'
        buffer_data = np.zeros((15, data_num), dtype=np.float64)
        if not (self.serial and self.serial.is_open):
            return buffer_data

        self.serial.timeout = timeout_sec
        raw_buffer = bytearray()
        raw_buffer.extend(self.infinite_iir_buffer) # 默认留置缓存数据
        valid_frames = 0
        start_time = time.time()
        last_counter = None  # 上一个成功识别的帧计数器

        valid_bytes = 0
        all_bytes = 0
        while valid_frames < data_num:
            # 读取剩余需要的字节数，或多读一些以便同步
            to_read = max(FRAME_SIZE * (data_num - valid_frames), self.bulk_buffer_packet_size)
            ...
            if not chunk:
                ...
                if (time.time() - start_time) > timeout_sec:
                    break
                continue
            # print('接收到数据：', str_to_hexstr(chunk))
            raw_buffer.extend(chunk)
            # 帧同步
            cursor = 0
            while len(raw_buffer) - cursor >= FRAME_SIZE and valid_frames < data_num:
                # print(f'当前Cursor位置：{cursor}/{len(raw_buffer)}')
                # 查找帧头
                if raw_buffer[cursor:cursor+2] != FRAME_HEAD:
                    # print(f'未匹配帧头 {str_to_hexstr(raw_buffer[cursor:cursor+2])}')
                    cursor += 1
                    all_bytes += 1
                    continue
                # print(f'找到帧头')
                # 检查帧尾
                if raw_buffer[cursor+82:cursor+83] != FRAME_TAIL:
                    print(f'未匹配帧尾 {str_to_hexstr(raw_buffer[cursor+83:cursor+84])}')
                    cursor += 1
                    all_bytes += 1
                    continue
                # print(f'找到帧尾')
                ...
                frame = raw_buffer[cursor:cursor+FRAME_SIZE]
                current_counter = frame[83]  # 获取当前帧计数器
                ...
                if last_counter is not None:
                    ...
                    if current_counter != expected_counter:
                        # 计算缺失的帧数
                        if current_counter > expected_counter:
                            missing_frames = current_counter - expected_counter
                        else:
                            missing_frames = 256 - expected_counter + current_counter
                        
                        # 填充缺失帧为零
                        for _ in range(min(missing_frames, data_num - valid_frames)):
                            if valid_frames >= data_num:
                                break
                            # 缺失帧的所有数据保持为零（buffer_data已初始化为零）
                            valid_frames += 1
                ...
                if valid_frames < data_num:
                    ...
                    for i in range(12):
                        start = 2 + i * 6
                        val = int.from_bytes(frame[start:start+6], byteorder='big', signed=True)
                        # 归一化到±0.5幅度
                        buffer_data[i, valid_frames] = val / (2**48)
                    # 解析2组4字节直流分量
                    for i in range(2):
                        start = 2 + 12*6 + i*4
                        val = int.from_bytes(frame[start + 1:start+4], byteorder='big', signed=True)
                        buffer_data[12+i, valid_frames] = val / (2**24)
                    # 解析1字节计数器
                    buffer_data[14, valid_frames] = current_counter

                    # print(f'识别计数器：{int(current_counter)}')
            ...
            # 丢弃已处理的数据
            if cursor > 0:
                raw_buffer = raw_buffer[cursor:]
        self.infinite_iir_buffer = bytes(raw_buffer) # 把残余字节更新到缓存区里，连续采集中使用
        # print(f'完成一次IIR读取，有效帧数{valid_frames}/{data_num}，有效字节数：{valid_bytes}/{all_bytes}')
        return buffer_data
    
    def DReadPortPID(self, data_num: int, timeout_sec: float = 1.5):
        """读取指定长度数据，返回shape为(data_num, 15)的浮点数组"""
        # IIR数据包格式：单帧76字节，帧头\xfe\xdc，12组6字节交流分量，2组4字节直流分量，1字节计数器，帧尾\xff
        FRAME_SIZE = 76
        FRAME_HEAD = b'\xfe\xdc'
        FRAME_TAIL = b'\xff'
        buffer_data = np.zeros((12, data_num), dtype=np.float64)
        if not (self.serial and self.serial.is_open):
            return buffer_data
...
class EthernetController:
    ...
    def connect(self):
        ...
        if self.connected:
            return True
        # print("失联次数", self.count)
        try:
            self.sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            self.sock.settimeout(2.0)  # 连接超时
            self.sock.connect((self.ip, self.port))
            self.sock.settimeout(self.timeout)  # 设置常规接收超时
            self.connected = True
    ...
    def DWritePort(self, data):
        ...
        if not self.connected:
            raise ConnectionError(f"未连接到IP地址：{self.ip}:{self.port}")
        ...
        try:
            ...
            self.connected = False
            raise ConnectionError(f"TCP指令发送失败: {str(e)}")
    ...
    def DReadPort(self, size=84):
        ...
        """
        if not self.connected:
            raise ConnectionError(f"TCP未连接：{self.ip}:{self.port}")

        data = bytearray()
        start_time = None

        while len(data) < size:
            # 使用select检查数据是否可读（立即返回）
            ...
            try:
                ...
                if not chunk:
                    # 连接已关闭
                    self.connected = False
                    raise ConnectionError("TCP连接被关闭")
                ...
                self.connected = False
                raise ConnectionError(f"TCP数据接收失败: {str(e)}")
        #    print(len(data))
        #print(len(data))
        return bytes(data)
    ...
    def DReadPortIIR(self, data_num: int, timeout_sec: float = 1.5):
        ...
        while valid_frames < data_num:
            # 读取剩余需要的字节数，或读取最大包大小
            to_read = min(FRAME_SIZE * (data_num - valid_frames), self.bulk_buffer_packet_size)
    ...
    def DReadPortPID(self, data_num: int, timeout_sec: float = 1.5):
        """读取指定长度数据，返回shape为(data_num, 15)的浮点数组"""
        # PID数据包格式：单帧76字节，帧头\x55\x55，2通道32字节数据（r,x,y,error（8bytes）,feedback（6bytes）），1字节计数器，帧尾\xaa
        FRAME_SIZE = 76
        ...
        raw_buffer = bytearray()
        raw_buffer.extend(self.infinite_pid_buffer) # 默认留置缓存数据
        valid_frames = 0
        start_time = time.time()
        last_counter = None  # 上一个成功识别的帧计数器
        ...
        while valid_frames < data_num:
            ...
            while len(raw_buffer) - cursor >= FRAME_SIZE and valid_frames < data_num:
                ...
                # print(f'找到帧头')
                # 检查帧尾
                if raw_buffer[cursor+75:cursor+76] != FRAME_TAIL:
                    print(f'未匹配帧尾 {str_to_hexstr(raw_buffer[cursor+75:cursor+76])}')
                    cursor += 1
                    all_bytes += 1
                    continue
                # print(f'找到帧尾')
                ...
                frame = raw_buffer[cursor:cursor+FRAME_SIZE]
                current_counter = frame[74]  # 获取当前帧计数器
                ...
                if valid_frames < data_num:
                    # 解析PID数据分量 - 通道1
                    buffer_data[0, valid_frames] = int.from_bytes(frame[2:8], byteorder='big', signed=False) / 2 ** 48 # R
                    buffer_data[2, valid_frames] = int.from_bytes(frame[8:14], byteorder='big', signed=True) / 2 ** 48 # Y
                    buffer_data[1, valid_frames] = int.from_bytes(frame[14:20], byteorder='big', signed=True) / 2 ** 48 # X
                    buffer_data[3, valid_frames] = int.from_bytes(frame[20:28], byteorder='big', signed=True) / 2 ** 64 # error
                    buffer_data[4, valid_frames] = int.from_bytes(frame[28:34], byteorder='big', signed=False) * 2 / 1048576 + 2.6e9 # feedback
                    # 解析PID数据分量 - 通道2
                    buffer_data[5, valid_frames] = int.from_bytes(frame[34:40], byteorder='big', signed=False) / 2 ** 48 # R
                    buffer_data[7, valid_frames] = int.from_bytes(frame[40:46], byteorder='big', signed=True) / 2 ** 48 # Y
                    buffer_data[6, valid_frames] = int.from_bytes(frame[46:52], byteorder='big', signed=True) / 2 ** 48 # X
                    buffer_data[8, valid_frames] = int.from_bytes(frame[52:60], byteorder='big', signed=True) / 2 ** 64 # error
                    buffer_data[9, valid_frames] = int.from_bytes(frame[60:66], byteorder='big', signed=False) * 2 / 1048576 + 2.6e9 # feedback
                    

                    # 直流数据
                    ch1_data = bytes_to_num(frame[66:70]) % 2 ** 28
                    ch2_data = bytes_to_num(frame[70:74]) % 2 ** 28
                    if ch1_data > 2 ** 27 - 1:
                    ...
                    buffer_data[10, valid_frames] = ch1_data_buf # Fluo_DC
                    buffer_data[11, valid_frames] = ch2_data_buf # Laser_DC
            ...
            if cursor > 0:
                raw_buffer = raw_buffer[cursor:]
        self.infinite_pid_buffer = bytes(raw_buffer) # 把残余字节更新到缓存区里，连续采集中使用
        # print(f'完成一次IIR读取，有效帧数{valid_frames}/{data_num}，有效字节数：{valid_bytes}/{all_bytes}')
        return buffer_data
...
class LIA_API(EthernetController):
    def __init__(self, *args, **kwargs):
        EthernetController.__init__(self, *args, **kwargs)
        # todo: 注册新设备类型
        self.dev_ctrl = {
            'LIA': self.lia_play,
            'MW': self.mw_play,
    ...
    def set_param(self, name, value, *args, **kwargs):
        ...
        if name == 'laser_power':
            ...
            # TODO: CH3随着CH2一起改！
            if ch == 2:
                logging.info("临时方案 - 同时设置CH2和CH3的调制和解调频率")
                self.Modu_fre_config(3, value)
                self.De_fre_config(3, value)
            ...
            return_value = value
        # logging.info(f'鎖相放大器设备层面，设置[{name}], 设置值[{value}]， 返回值[{return_value}]')
        time.sleep(0.05)
        return return_value
    ...
    def start_infinite_iir_acq(self):
        ...
        time.sleep(6 * self.tc + 0.05)  # 等待6个tc
    ...
    def stop_infinite_iir_acq(self):
        '''停止无限IIR采集模式'''
        self.DWritePort(b'\xAB\xAB')
        time.sleep(0.05)
        self.flush_input_buffer()

    def start_infinite_pid_acq(self):
        '''启动无限PID采集模式'''
        # 使用前需要确保系统处于IDLE状态
        self.infinite_pid_buffer = b''
        self.DWritePort(b'\x00\x00')
    ...
    def stop_infinite_pid_acq(self):
        '''停止无限PID采集模式'''
        # TODO: 在修改固件前先用大量的\x00\x00字节退出无限采集模式
        self.DWritePort(b'\x00\x00' * 100)
        time.sleep(0.05)
        self.flush_input_buffer()

    def get_infinite_pid_points_deprecated(self, data_num):
        '''获取无限PID采集模式下的数据，格式：(1R, 1X, 1Y, 1Error, 1Feedback, 2R, 2X, 2Y, 2Error, 2Feedback, Fluo_DC, Laser_DC)'''
        # print('无限采集模式 - 完成PID采集指令下发，准备接受PID数据')
        PID_data = self.DReadPort(data_num * 76)  # 76 bytes per point
        # print(f'无限采集模式 - 接收到PID数据')
        
        data = [[] for i in range(12)]  # 12个数据通道
        ...
        for i in range(data_num):
            ...
            # 通道2数据
            iir_data_r2 = bytes_to_num(PID_data[76 * i + 34: 76 * i + 40])
            iir_data_x2 = bytes_to_num(PID_data[76 * i + 40: 76 * i + 46])
            iir_data_y2 = bytes_to_num(PID_data[76 * i + 46: 76 * i + 52])
            error_buf2 = bytes_to_num(PID_data[76 * i + 52: 76 * i + 60])
            feedback_buf2 = bytes_to_num(PID_data[76 * i + 60: 76 * i + 66])
            
            # 通道2数据处理
            ch2_iir_data_r = iir_data_r2 / 2 ** 48.0
            if iir_data_y2 > 2 ** 47 - 1:
                ch2_iir_data_y = (iir_data_y2 - 2 ** 48.0) / 2 ** 48.0
            else:
                ch2_iir_data_y = iir_data_y2 / 2 ** 48.0
            if iir_data_x2 > 2 ** 47 - 1:
                ch2_iir_data_x = (iir_data_x2 - 2 ** 48.0) / 2 ** 48.0
            else:
                ch2_iir_data_x = iir_data_x2 / 2 ** 48.0
            if error_buf2 > 2 ** 63 - 1:
                ch2_error_buf = (error_buf2 - 2 ** 64.0) / 2 ** 64.0
            else:
                ch2_error_buf = error_buf2 / 2 ** 64.0
            ch2_feedback_buf = feedback_buf2 * 2 / 1048576 + 2.6 * 1e9
            ...
            # 存储数据
            data[0].append(ch1_iir_data_r)   # 1R
            data[1].append(ch1_iir_data_x)   # 1X
            data[2].append(ch1_iir_data_y)   # 1Y
            data[3].append(ch1_error_buf)    # 1Error
            data[4].append(ch1_feedback_buf) # 1Feedback
            data[5].append(ch2_iir_data_r)   # 2R
            data[6].append(ch2_iir_data_x)   # 2X
            data[7].append(ch2_iir_data_y)   # 2Y
            data[8].append(ch2_error_buf)    # 2Error
            data[9].append(ch2_feedback_buf) # 2Feedback
            data[10].append(ch1_data_buf)    # Fluo_DC
            data[11].append(ch2_data_buf)    # Laser_DC
    ...
    def start_finite_pid_acq(self, data_num):
        '''启动有限PID采集模式'''
        # 使用前需要确保系统处于IDLE状态
        self.DWritePort(b'\x00\x00')
        self.DWritePort(b'\x00\x04')  # 进入到IIR状态
    ...
    def get_infinite_pid_points(self, data_num):
        '''获取无限PID采集模式下的数据，格式：(1R, 1X, 1Y, 1Error, 1Feedback, 2R, 2X, 2Y, 2Error, 2Feedback, Fluo_DC, Laser_DC)'''
        # print('无限采集模式 - 完成PID采集指令下发，准备接受PID数据')
        PID_data = self.DReadPortPID(data_num)  # 76 bytes per point
        # print(f'无限采集模式 - 接收到PID数据')
        return PID_data
    ...
    def get_infinite_iir_points(self, data_num):
        ...
        # print('无限采集模式 - 完成IIR采集指令下发，准备接受IIR数据')
        IIR_data_decoded = self.DReadPortIIR(data_num)  # 80 byte
        # print(f'无限采集模式 - 接收到IIR数据')
        data = IIR_data_decoded[:14]
        return data
    
    def start_finite_iir_acq(self, data_num):
        '''启动有限IIR采集模式'''
        # 使用前需要确保系统处于IDLE状态
        self.DWritePort(b'\x00\x00')
        self.DWritePort(b'\x00\x04')  # 进入到IIR状态
    ...
    def get_finite_iir_acq_points(self, total_data_num, current_data_num, data_num):
        '''获取有限IIR采集模式下的数据，[r x y] * 4 + 2 = 14列'''
        data_num = min(data_num, total_data_num - current_data_num)
    ...
    def IIR_play(self, data_num, CW_mode=False):
        ...
        self.DWritePort(num_to_bytes(data_num, 4))
        time.sleep(0.05)
        time.sleep(6 * self.tc)
        IIR_data = b''
        # print('完成IIR采集指令下发，准备接受IIR数据')
        IIR_data_decoded = self.DReadPortIIR(data_num)  # 80 byte
        time.sleep(0.05)
        # print(f'接收到IIR数据')
        data = IIR_data_decoded[:14]
    ...
    def MW_SPI_Ctrl(self, ch1_Fre_buf, ch1_modu, ch1_atte, ch2_Fre_buf, ch2_modu, ch2_atte):
        ...
        time.sleep(0.05)
        self.DWritePort(b'\xab\xcd')
        self.DWritePort(b'\x00\xb1')
        time.sleep(0.05)
    ...
    def error_check(self, data_num):
        ...
        for i in range(data_num):
            ...
            print(f'i={i}/{data_num}, data_buf={data_buf}')
...
if __name__ == '__main__':
    LIA_mini_API = LIA_API(port='COM1')
